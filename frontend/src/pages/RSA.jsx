...
const RSA = () => {
  ...
  // Gọi API pow khi cần
  const handlePowRequest = async () => {
    const data = { base: 2, exponent: 10, modulus: 13 }; // Ví dụ
    try {
      const result = await CommonAPI.pow(data);
      setPowResult(result); // Lưu kết quả trả về từ API vào state
    } catch (error) {
      console.error("Error in pow API:", error);
    }
  };
  ...
  return (
    <div>
      ...
      <div>
        <h3>Test Pow API</h3>
        <button onClick={handlePowRequest}>Call Pow API</button>
        <p>Result from Pow API: {powResult ? powResult.result : "No result"}</p>
      </div>
    </div>
  );
};
